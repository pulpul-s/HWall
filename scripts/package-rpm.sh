...
# Override with FEDORA_IMAGE=... when deliberately targeting another release.
IMAGE=${FEDORA_IMAGE:-registry.fedoraproject.org/fedora:44}
CONTAINER_NAME="hwall-rpm-build-$$-$RANDOM"
WORK_DIR=$(mktemp -d "${TMPDIR:-/tmp}/hwall-rpm-package.XXXXXXXX")
STAGE_DIR="$WORK_DIR/stage"
TARGET_DIR="$WORK_DIR/cargo-target"
NFPM_WORK_DIR="$WORK_DIR/nfpm-work"
PACKAGE_DIR="$WORK_DIR/package"
FINAL_DIR="$ROOT/dist/rpm"
...
cleanup() {
  ...
  # Remove a container left by an interrupted or failed run.
  podman rm --force "$CONTAINER_NAME" >/dev/null 2>&1 || true

  # The requested default is zero retained build images. This deliberately
  # removes the Fedora image even when it existed before this invocation.
...
cp "$ROOT/packaging/nfpm/hwall-rpm.yaml" \
  "$NFPM_WORK_DIR/packaging/nfpm/hwall-rpm.yaml"
ln -s "$STAGE_DIR" "$NFPM_WORK_DIR/dist/rpm-root"
...
podman run \
  ...
  bash -euxo pipefail -c '
    dnf install -y \
      ca-certificates curl gcc make gtk4-devel pkgconf-pkg-config file binutils
...
(
  cd "$NFPM_WORK_DIR"
  "$NFPM_BIN" package \
    --config packaging/nfpm/hwall-rpm.yaml \
    --packager rpm \
    --target "$PACKAGE_DIR/"
)

PACKAGE_FILE=$(find "$PACKAGE_DIR" -maxdepth 1 -type f -name '*.rpm' -print -quit)
[[ -n "$PACKAGE_FILE" ]] || {
  echo "error: nFPM did not produce an RPM package" >&2
  exit 1
}
...
echo "RPM package written to: $FINAL_DIR/$PACKAGE_NAME"
echo "Cleanup complete: container, Fedora image, and temporary build files removed."
